...
Generated by `preprocess_and_extract_hpc.py`.  
**Data source:** Raw BIDS EEG files — full preprocessing applied per-subject (bandpass → ASR → ICA+ICLabel).  
...
### Step 1 — What file should I look at first?
...
### Step 2 — What do the groups mean?
...
> **Priority biomarkers — use these first in your reasoning pipeline:**
>
...
- DTABR separates CN clearly from both dementia groups, but does not distinguish AD from FTD well — both are ~31.
- theta_alpha_ratio better separates AD from FTD (AD is higher).
- alpha2_abs shows FTD has lower alpha-2 power than AD, which may reflect FTD's frontal-dominant pathology disrupting posterior alpha differently than AD.
- ms_B_coverage is consistently highest in FTD across all conditions, which may reflect right-hemisphere network disruption specific to FTD.
