...
Dataset: OpenNeuro ds006036 (Alzheimer's/FTD/CN eyes-open photic stimulation EEG)

Companion / complementary dataset to ds004504.
Same cohort, same subjects — but eyes-OPEN with photic stimulation.

Key differences vs ds004504 version:
  - Task name    : "photomark"  (was "eyesclosed")
  - Derivatives  : derivatives/eeglab/sub-XXX/eeg/  (was derivatives/sub-XXX/eeg/)
  - Preprocessing: Butterworth 0.5–45 Hz + ASR + ICA already applied by authors

Input:  {BASE_DIR}/derivatives/eeglab/sub-XXX/eeg/sub-XXX_task-photomark_eeg.set
Output: {BASE_DIR}/results/SampleEntropy.csv

Usage:
    python 02_sample_entropy_ds006036.py --base_dir /path/to/ds006036
    python 02_sample_entropy_ds006036.py --base_dir /path/to/ds006036 --subject sub-001
...
# ds006036-specific: task name and derivatives sub-path
TASK_NAME         = "photomark"
DERIVATIVES_SUBDIR = "eeglab"   # derivatives/eeglab/sub-XXX/eeg/
...
def find_set_files(base_dir: str, subject: str = None) -> list:
    """
    ds006036 stores preprocessed data under derivatives/eeglab/sub-XXX/eeg/.
    File naming: sub-XXX_task-photomark_eeg.set
    """
    if subject:
        pattern = os.path.join(
            base_dir, "derivatives", DERIVATIVES_SUBDIR, subject, "eeg",
            f"{subject}_task-{TASK_NAME}_eeg.set"
        )
    else:
        pattern = os.path.join(
            base_dir, "derivatives", DERIVATIVES_SUBDIR, "sub-*", "eeg",
...
def sampen_single(signal: np.ndarray) -> float:
    """Compute Sample Entropy for a 1D signal. Returns np.nan on failure."""
    std = signal.std()
    if std < 1e-10:
        return np.nan
    try:
...
def process_subject(set_path: str) -> dict:
    ...
    # Notch filter — data is already bandpass-filtered (0.5–45 Hz) by authors,
    # but line noise notch is still worthwhile for this eyes-open dataset.
...
def main():
    parser = argparse.ArgumentParser(
        description="Extract Sample Entropy from ds006036 EEG dataset."
    )
    ...
    if not set_files:
        print(f"[ERROR] No .set files found under {base_dir}/derivatives/{DERIVATIVES_SUBDIR}/")
        print(f"        Expected pattern: sub-XXX_task-{TASK_NAME}_eeg.set")
        return
