...
⚠️  IMPORTANT NOTE ON VALIDITY:
    ...
    Sample Entropy is highly sensitive to driven periodic activity: a strong
    40 Hz ASSR makes the signal more predictable and lowers SampEn for
    reasons that have NOTHING to do with disease. Computing SampEn on the
    full recording confounds stimulation effects with group differences.

    Three modes are available (--mode):
      'rest'   — use only OFF epochs from events.tsv  (RECOMMENDED, default)
      'full'   — use the entire recording (legacy, contamination expected)
...
Key differences vs ds006036/ds004504 versions:
  ...
  - Reference      : average reference (applied during preprocessing)
  - Sampling rate  : 250 Hz
  - Preprocessing  : Makoto pipeline already applied (HPF 1 Hz, line noise,
                     bad-channel rejection + interpolation, ASR, ICA, dipole rejection)

Input:  {BASE_DIR}/sub-XX/eeg/sub-XX_task-40HzAuditoryEntrainment_eeg.set
Output: {BASE_DIR}/results/SampleEntropy_ds005048.csv

Usage:
    python 02_sample_entropy_ds005048.py --base_dir /path/to/ds005048
    python 02_sample_entropy_ds005048.py --base_dir /path/to/ds005048 --subject sub-01
    python 02_sample_entropy_ds005048.py --base_dir /path/to/ds005048 --mode full
...
# Channel groups for regional analysis (standard 10-20 names)
CHANNEL_GROUPS = {
...
SAMPEN_R = 0.2        # Tolerance as fraction of signal std
...
REST_CODE = "1"                  # events.tsv: 1 = rest start, 2 = stimulus start
STIM_CODE = "2"
...
def get_rest_intervals(events_df: pd.DataFrame, recording_dur: float) -> list:
    """Return list of (onset, offset) tuples for REST (OFF) periods."""
    if events_df.empty:
        return []
    label_col = None
    ...
    for i, lab in enumerate(labels):
        if lab == REST_CODE:
            start = float(onsets[i]) + 1.0  # trim 1 s for transient
...
def select_segments(raw: mne.io.BaseRaw, set_path: str, mode: str):
    """Return raw cropped to the chosen analysis segment(s)."""
    ...
    # full mode (or rest-fallback) — crop edges for stationarity
...
def compute_regional_sampen(epochs: np.ndarray, raw: mne.io.BaseRaw) -> dict:
...
def process_subject(set_path: str, mode: str = "rest") -> dict:
    ...
    # Notch filter (Tehran mains = 50 Hz)
    ...
    regional = compute_regional_sampen(epochs, raw_seg)
...
def main():
    parser = argparse.ArgumentParser(
        description="Extract Sample Entropy from ds005048 EEG dataset."
    )
    ...
    parser.add_argument("--output", default=None,
                        help="Output CSV path. Default: {BASE_DIR}/results/SampleEntropy_ds005048.csv")
    parser.add_argument("--mode", default=ANALYSIS_MODE, choices=["rest", "full"],
                        help="'rest' uses OFF epochs from events.tsv (recommended). "
                             "'full' uses the entire recording (legacy).")
    args = parser.parse_args()

    base_dir   = os.path.abspath(args.base_dir)
    output_csv = args.output or os.path.join(base_dir, "results", "SampleEntropy_ds005048.csv")
    ...
    print(f"Found {len(set_files)} subject(s). Mode: {args.mode}. Starting SampEn extraction...\n")
    ...
    if group_col and metric in df.columns:
        print(f"\n--- Posterior SampEn by {group_col} (mode={args.mode}) ---")
